...
DEFAULT_INPUT = PROJECT_ROOT / "images" / "navigation" / "001-007.png"
...
def render_frame(source: Image.Image, index: int, total_frames: int) -> Image.Image:
    """Render one undistorted 9:16 frame focused on visitor-environment interaction."""
    ...
    # The visitor stays in frame while the crop drifts slightly toward the screen.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build cinematic scene 002 from a still mall image."
    )
    parser.add_argument(
        "--input",
        "-i",
