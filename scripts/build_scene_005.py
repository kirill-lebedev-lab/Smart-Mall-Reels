...
DEFAULT_INPUT = PROJECT_ROOT / "images" / "navigation" / "001-010.png"
DEFAULT_OUTPUT = PROJECT_ROOT / "scenes" / "scene_005.mp4"
...
def render_frame(source: Image.Image, index: int, total_frames: int) -> Image.Image:
    """Render one undistorted 9:16 frame for the visitor's choice moment."""
    ...
    # Preserve continuity with scene 004 while gently emphasizing the interaction.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build cinematic scene 005 from a still mall image."
    )
    parser.add_argument(
        "--input",
        "-i",
